...
pipelines from YAML while keeping projector/applier binding explicit in host
code.
...
- [x] Add a concrete host-binary example that registers a real projector/applier pair and invokes `deltaflow run`.
- [ ] Add host-binary embedding documentation for runtime registry setup.
...
- Host binaries explicitly register projector factories by `pipelines[].projector.name`.
- Host binaries explicitly register applier factories by `pipelines[].target.type`.
...
- Requires explicit runtime registry wiring from a host binary.
...
- [ ] README and docs include host-embedding example for runtime registrations.
